...
export default class Enemy extends cc.Component {
    ...
    enemyMovement(dt) {
        ...
        if(this.stop && this.getComponent(cc.RigidBody).linearVelocity.y < 10 && this.getComponent(cc.RigidBody).linearVelocity.y > -10 && this.onGround && this.jump) {
            this.onGround = false;
            this.jump = false;
            this.startCountdown(0.75);
    ...
    onBeginContact(contact, self, other) {
        if(other.node.name == "leftBound" && !this.enemyDead) {
            ...
            this.node.getComponent(cc.RigidBody).linearVelocity = cc.v2(-50, 0);
            this.direction = 1;
        } else if(other.node.name == "collidable" && !this.enemyDead) {
            this.onGround = true;
